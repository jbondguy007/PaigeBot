...
async def ssadmin(ctx, arg=None):
    ...
    if arg.lower() == 'list':
        ...
        while True:
            abubis_assigned = registrations[str(abubis_id)]["assigned"]

            # Find who assigned Abubis
            abubis_secret_santa_list = [
                registrations[uid]["info"]
                for uid in registrations
                if registrations[uid]["assigned"]["id"] == abubis_id
            ]

            # Safety check — if somehow no one was assigned Abubis
            if not abubis_secret_santa_list:
                await ctx.send("Error: No one assigned Abubis. Re-rolling...")
                roll_assignments()
                continue

            abubis_secret_santa = abubis_secret_santa_list[0]

            # Check conflict
            if (
                abubis_assigned["id"] in [ray_id, packet_id]
                or abubis_secret_santa["id"] in [ray_id, packet_id]
            ):
                await ctx.send("Re-rolling due to conflict...")

                # Clear assigned entries
                for userid in registrations:
                    registrations[userid]['assigned'] = {}

                roll_assignments()
                continue  # Try again

            # If we reach here, roll is valid
            break

        with open("secret_santa_registration.json", "w") as f:
            json.dump(registrations, f, indent=4)

        await ctx.send(f"Final assignments completed and saved.")
